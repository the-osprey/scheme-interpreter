...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(fn)
        return Procedure.eval_call(fn, rest, env)
...
def eval_all(expressions, env):
    ...
    def helper(expressions, env, length):
        if not expressions:
            return
        if length == 1:
            ...
            return helper(expressions.second, env, length - 1)
    return helper(expressions, env, expressions.__len__())
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        #use complete_eval to find Thunk objects
        #remember how macros evaluate operands!
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        symbol, formals, body = target.first, target.second, expressions.second
...
def do_and_form(expressions, env):
    ...
    length = expressions.__len__()
    if not expressions:
        return True
    while length > 1:
        if not scheme_truep(scheme_eval(expressions.first, env)):
            return False
        #inserted expressions.second case do deal with tail recursion
...
def do_or_form(expressions, env):
    ...
    while length >= 1:
        #inserted expressions.second case do deal with tail recursion
        if expressions.second is nil:
            outcome = scheme_eval(expressions.first, env, True)
        else:
            outcome = scheme_eval(expressions.first, env)
        if scheme_truep(outcome):
            return outcome
        expressions = expressions.second
        length -= 1
    return False
...
def do_define_macro(expressions, env):
    ...
    #notice this is effectively the do_define_form, but calling MacroProcedure instead
    check_form(expressions, 2)
    target = expressions.first
